...
import { Calendar, Copy, Check, Share2, ChevronDown, X } from 'lucide-react';
...
import { CalendarFeedGuideSheet } from './CalendarFeedGuideSheet';
...
export const CalendarFeedPanel = ({
  ...
  const onConnect = async () => {
    await copyFeedUrl(feedUrl);
    setShowGuide(true);
    track('calendar_feed_subscribe');
  };

  const onGuideOpenedGoogle = () => {
    markCalendarFeedDone();
    onSubscribed?.();
  };
  ...
  const guide = (
    <CalendarFeedGuideSheet
      open={showGuide}
      feedUrl={feedUrl}
      onClose={() => setShowGuide(false)}
      onOpenedGoogle={onGuideOpenedGoogle}
    />
  );
