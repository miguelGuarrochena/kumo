...
export const FullReminderSheet = ({
  open,
  reminder,
  contacts,
  hasWa,
  onClose,
}: FullReminderSheetProps) => {
  ...
  const onSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    startTransition(async () => {
      ...
      if (result.ok) {
        toast.success(
          reminder ? t.calendar.reminder_updated : t.calendar.reminder_created,
        );
        if (result.syncWarning) toast.warning(result.syncWarning);
        if (!reminder) {
          track("reminder_created", {
            type,
            contacts_count: notifyContactIds.length,
          });
        }
        router.refresh();
        onClose();
        ...
        <label className="flex items-center gap-2 text-sm border border-slate-200 dark:border-slate-700 rounded-xl p-3">
          ...
          {type === "birthday" && (
            <span className="ml-auto text-xs text-slate-400">
              {t.calendar.recurring_recommended}
            </span>
          )}
        </label>
            ...
            <div className="space-y-1.5">
              {contacts.map((c) => {
                ...
                return (
                  ...
                  >
                    <div
                      className={`w-5 h-5 rounded border-2 grid place-items-center transition-all ${
                        selected
